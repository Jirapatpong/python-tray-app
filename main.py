...
import queue
 ...
 class App:
     ...
     def on_app_quit(self):
         ...
         # --- NEW: Clean up the lock file on exit ---
         try:
 ...
 if __name__ == "__main__":
     ...
     if is_admin():
         try:
             root = tk.Tk()
             # Pass the lock file path to the App instance
             ...
             # If the main app fails, make sure to clean up the lock file
             if os.path.exists(lock_file):
                 os.remove(lock_file)
             messagebox.showerror("Application Error", f"An unexpected error occurred: {e}")
     else:
         # If not admin, remove the lock file before re-launching
         if os.path.exists(lock_file):
             os.remove(lock_file)
