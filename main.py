...
# VERSION: 1.0.4 (Compact UI + Zip/Stream Final)
import subprocess
import threading
import os
import sys
...
class App:
    ...
    def __init__(self, master, lock_file_path):
        ...
        # --- UI UPDATE: Compact Size ---
        app_width = 600  # Slightly wider
        app_height = 520 # Shorter (Compact)
    ...
    def create_side_button(self, parent, text, command):
        import tkinter as tk
        # UI UPDATE: Reduced Padding for Compact Sidebar
    ...
    def create_widgets(self):
        ...
        # UI UPDATE: Reduced Title Padding
        ...
        # UI UPDATE: Reduced Content Padding (20 -> 15)
    ...
    def _start_stream(self):
        ...
        # --- FIX: Use default 800px if hidden (tab switched), else real height ---
        target_height = self.stream_embed_frame.winfo_height()
        if target_height < 200: target_height = 600 

        # --- FIX: Set ADB env variable ---
        env = os.environ.copy(); env['ADB'] = self.ADB_PATH

        # --- KEY FIX: NO --max-size ---
        # We let scrcpy launch full size, then we shrink it using MoveWindow.
        # This prevents "tiny window" issues if scrcpy decides 1px is funny.
    ...
    def _resize_stream_to_fit(self, event=None):
        ...
        # Calc new size (Fit Height first)
        nh = fh
        nw = int(nh * self._source_aspect)
        # If width overflow, Fit Width
        if nw > fw:
            nw = fw
    ...
    def process_zip_file(self, zp):
        ...
        tmp = os.path.join(self.base_path, "tmp", f"extract_{os.path.basename(zp)}")
        orig_name = os.path.basename(zp)
        orig_dir = os.path.dirname(zp)
        parts = orig_name.replace('.zip', '').split('-')
        new_zp = zp
        if len(parts) == 5:
             new_name = f"{parts[0]}-{parts[1]}-{parts[2]}-{parts[3]}-{time.strftime('%y%m%d')}-{parts[4]}.zip"
             new_zp = os.path.join(orig_dir, new_name)
        ...
        try:
            ...
            with zipfile.ZipFile(new_zp, 'w', zipfile.ZIP_DEFLATED) as z:
                for root, dirs, files in os.walk(tmp):
                    for d in dirs:
                        p = os.path.join(root, d); arc = os.path.relpath(p, tmp).replace(os.sep, '/') + '/'
