...
class App:
    ...
    def __init__(self, master, lock_file_path):
        ...
        self.style.map(
            'Treeview',
            background=[('selected', self.COLOR_SHADOW_DARK)],
            foreground=[('selected', self.COLOR_SHADOW_LIGHT)]
        )
        self.style.configure(
            'Treeview.Heading',
        ...
        # Monitor device connection in background
        ...
        # Setup logging for API tab
        self._setup_log_file()
        self._periodic_log_save()

        # Load config and start file watchers
        self._start_monitoring_services()

        # Scan any existing APK files in folder (for auto-install)
    ...
    def create_widgets(self):
        ...
        # --- 1. Sidebar ---
        sidebar_frame = tk.Frame(self.master, bg=self.COLOR_SIDEBAR_BG, width=170)
        ...
        self.side_btn_device = self.create_side_button(
            sidebar_frame, "Device Status", lambda: self.switch_tab('device')
        )
        self.side_btn_device.pack(fill='x')

        self.side_btn_api = self.create_side_button(
            sidebar_frame, "API Log", lambda: self.switch_tab('api')
        )
        self.side_btn_api.pack(fill='x')

        self.side_btn_zip = self.create_side_button(
            sidebar_frame, "Zip Monitor", lambda: self.switch_tab('zip')
        )
        self.side_btn_zip.pack(fill='x')

        self.side_btn_apk = self.create_side_button(
            sidebar_frame, "APK Monitor", lambda: self.switch_tab('apk')
        )
        self.side_btn_apk.pack(fill='x')

        self.side_btn_stream = self.create_side_button(
            sidebar_frame, "Stream Screen", lambda: self.switch_tab('stream')
        )
        self.side_btn_stream.pack(fill='x')

        self.all_side_buttons = [
            self.side_btn_device,
            self.side_btn_api,
            self.side_btn_zip,
            self.side_btn_apk,
            self.side_btn_stream
        ]
        ...
        self.refresh_button = self.create_neumorphic_button(
            buttons_frame, text="Refresh", command=self.refresh_devices
        )
        self.refresh_button.pack(side='left', padx=(0, 10))

        self.disconnect_button = self.create_neumorphic_button(
            buttons_frame, text="Disconnect", command=self.disconnect_device
        )
        self.disconnect_button.pack(side='left')
        self.disconnect_button.config(state='disabled')

        self.connect_button = self.create_neumorphic_button(
            buttons_frame, text="Connect", command=self.connect_device, is_accent=True
        )
        ...
        self.api_status_dot = tk.Canvas(
            api_header_frame, width=10, height=10, bg=self.COLOR_BG, highlightthickness=0
        )
        ...
        search_button = ttk.Button(
            api_header_frame, text="Search", style='Raised.TButton',
            command=self.search_api_logs
        )
        search_button.grid(row=0, column=3, sticky='e', padx=(0, 5))

        self.refresh_api_button = ttk.Button(
            api_header_frame, text="Restart API", style='Raised.TButton',
            command=self.refresh_api_exe
        )
        ...
        self.api_log_text = scrolledtext.ScrolledText(
            log_frame,
            wrap=tk.WORD,
            state='disabled',
            bg=self.COLOR_BG,
        ...
        self.zip_count_label = tk.Label(
            zip_header_frame,
            text="Total Files Processed: 0",
            font=('Segoe UI', 9, 'bold'),
            bg=self.COLOR_BG,
            fg=self.COLOR_TEXT
        )
        ...
        self.zip_tree = ttk.Treeview(
            self.zip_frame, columns=('filename', 'status'), show='headings'
        )
        self.zip_tree.heading('filename', text='FILENAME', anchor='w')
        self.zip_tree.heading('status', text='STATUS', anchor='w')
        self.zip_tree.column('filename', anchor='w', width=240)
        ...
        self.zip_tree.tag_configure(
            'processing', foreground=self.COLOR_WARNING, font=('Segoe UI', 9, 'bold')
        )
        self.zip_tree.tag_configure(
            'done', foreground=self.COLOR_SUCCESS, font=('Segoe UI', 9, 'bold')
        )
        self.zip_tree.tag_configure(
            'error', foreground=self.COLOR_DANGER, font=('Segoe UI', 9, 'bold')
        )
        ...
        self.apk_count_label = tk.Label(
            apk_header_frame,
            text="Total APKs Processed: 0",
            font=('Segoe UI', 9, 'bold'),
            bg=self.COLOR_BG,
            fg=self.COLOR_TEXT
        )
        ...
        self.apk_tree.tag_configure(
            'skipped', foreground=self.COLOR_TEXT, font=('Segoe UI', 9, 'italic')
        )
        ...
        # Black box for embedding scrcpy
        ...
        # Start stream button
        self.stream_start_btn = ttk.Button(
            self.stream_frame,
            text="Start Stream",
            style='Raised.TButton',
    ...
    def _auto_save_log(self):
        ...
        try:
            ...
            threading.Thread(
                target=self._save_log_to_file_worker,
                args=(content,),
                daemon=True
            ).start()
        except Exception as e:
    ...
    def _save_log_to_file_worker(self, content):
        try:
            ...
            if today_date_str != self.current_log_date:
                # date changed, flush old file fully, then start new day's file
                if content.strip():
                ...
                # clear widget text (on main thread)
                self.master.after(0, self._clear_api_log_widget)
            else:
                # same day, just overwrite with formatted content
                if content.strip():
                    formatted_content = self._format_sql_log(content)
                    with open(self.log_filepath, 'w', encoding='utf-8') as f:
                        f.write(formatted_content)
    ...
    def _periodic_log_save(self):
        if self.is_running:
            self._auto_save_log()
            self.master.after(30000, self._periodic_log_save)  # every 30 seconds
    ...
    def _format_sql_log(self, raw_content):
        ...
        for line in raw_content.splitlines():
            ...
            if ('SELECT ' in upper_line or
                'INSERT INTO ' in upper_line or
                'UPDATE ' in upper_line or
                'DELETE FROM ' in upper_line):
                formatted_lines.append("--- SQL Query ---")
                for keyword in sql_keywords:
                    upper_keyword = keyword.upper()
                    if upper_keyword in upper_line:
                        line = line.replace(keyword, f'\n  {keyword.strip()} ')
                        line = line.replace(keyword.strip(), f'\n  {keyword.strip()} ')
    ...
    def switch_tab(self, tab_name):
        ...
        # If leaving stream tab, ensure stream is stopped
        if tab_name != "stream":
            self._stop_stream()
        ...
        if tab_name == 'device':
            ...
            self.side_btn_device.config(
                bg=self.COLOR_SIDEBAR_BTN_ACTIVE,
                fg=self.COLOR_SIDEBAR_TEXT_ACTIVE
            )

        elif tab_name == 'api':
            self.api_frame.tkraise()
            self.side_btn_api.config(
                bg=self.COLOR_SIDEBAR_BTN_ACTIVE,
                fg=self.COLOR_SIDEBAR_TEXT_ACTIVE
            )
    ...
    def search_api_logs(self):
        ...
        if search_term:
            start_pos = self.api_log_text.search(
                search_term,
                self.last_search_pos,
                stopindex=tk.END,
                nocase=True
            )
    ...
    # -------------------------------------------------------------------------
    # Launch api.exe and process logs
    # -------------------------------------------------------------------------
    ...
    def update_tray_status(self):
        ...
        device_status_text = (
            f"Device: {self.connected_device}" if self.connected_device else "Device: Disconnected"
        )
        api_status_text = "API: Online" if self.api_status == "Online" else "API: Offline"
        ...
        if self.connected_device:
            ...
            self.tray_icon.title = "HHT Android Connect: Connected"
        else:
    ...
    def device_monitor_loop(self):
        while self.is_running:
            try:
                ...
                # 2) Auto-connect first newly plugged device (if none connected)
    ...
    def _refresh_devices(self):
        ...
        try:
            ...
            self.master.after(0, lambda: messagebox.showerror(
                "Error",
                f"An error occurred while refreshing devices:\n{e}"
            ))
        finally:
    ...
    def _connect_device(self, device_id):
        ...
        try:
            result = subprocess.run(
                [self.ADB_PATH, "-s", device_id, "reverse", "tcp:8000", "tcp:8000"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                creationflags=subprocess.CREATE_NO_WINDOW
            ...
            if self.connected_device is None and result.returncode == 0:
                ...
                self.master.after(0, self._clear_apk_monitor)
                self.master.after(100, self._scan_existing_apk_files)
    ...
    def check_adb(self):
        ...
        try:
            ...
            messagebox.showerror(
                "ADB Error",
                f"ADB executable not found at the expected path: {self.ADB_PATH}"
            )
            return False
        except Exception:
            return False
    ...
    def connect_device(self):
        ...
        if self.connected_device == selected_device:
            messagebox.showinfo(
                "Already Connected",
                f"Device {selected_device} is already connected."
            )
            return
    ...
    # -------------------------------------------------------------------------
    # Load Config.ini and start Zip/APK monitor
    # -------------------------------------------------------------------------
    def _load_configs(self):
        ...
        if not os.path.exists(config_path):
            ...
            messagebox.showerror(
                "Config Error",
                f"Configuration file not found. Please create it at:\n{config_path}"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror(
                    "Config Error",
                    "DEFAULT_PRICE_TAG_PATH not found in [SETTING] section of config.ini"
                )

            # APK Monitor path (MONITOR_PATH)
            try:
                self.apk_monitor_path = config['APK_INSTALLER']['MONITOR_PATH']
                if not os.path.exists(self.apk_monitor_path):
                    messagebox.showwarning(
                        "Config Warning",
                        f"The APK monitor path does not exist:\n{self.apk_monitor_path}\n\n"
                        f"The APK service will not start."
                    )
                else:
                ...
                messagebox.showerror(
                    "Config Error",
                    "MONITOR_PATH not found in [APK_INSTALLER] section of config.ini"
                )
    ...
    def _add_zip_to_monitor(self, filepath):
        ...
        item_id = self.zip_tree.insert(
            '', tk.END, values=(filename, 'Pending'),
            tags=('pending',)
        )
    ...
    def _update_zip_status(self, item_id, status):
        try:
            if not self.zip_tree.exists(item_id):
                return
    ...
    def process_zip_file(self, zip_path):
        ...
        # Wait until file is ready (no longer locked by writer)
        for _ in range(5):
            try:
        ...
        try:
            ...
            if len(filename_parts) == 5:
                # Insert missing export date: (prefix-branch-zero-date-time) -> add export_date before time
                ...
                new_filename = (
                    f"{part_prefix}-{part_branch}-{part_zero}-"
                    f"{part_date}-{part_export_date}-{part_time}.zip"
                )
                ...
                print(
                    f"Warning: Unknown filename format '{original_filename_with_ext}'. "
                    f"Re-zipping with original name."
                )
                new_zip_path = zip_path
            ...
            # Rebuild zip with directory fixes
    ...
    # -------------------------------------------------------------------------
    # APK monitor helpers (Auto Installer)
    # -------------------------------------------------------------------------
    ...
    def _update_apk_status(self, item_id, status_message):
        try:
            ...
            if ("Processing" in status_message or
                "Installing" in status_message or
                "Upgrading" in status_message or
                "Waiting" in status_message):
    ...
    # -------------------------------------------------------------------------
    # NEW: Screen Streaming Methods (scrcpy)
    # -------------------------------------------------------------------------
    def _start_stream(self):
        ...
        # Ensure scrcpy uses our bundled adb
        env = os.environ.copy()
    ...
    def _embed_stream_window(self):
        """Worker thread to find the scrcpy window and embed it into Tk frame with autoscale."""
        try:
            hwnd = 0
            retries = 20

            # Wait for scrcpy window with title HHT_STREAM to appear
            ...
            if hwnd == 0:
                ...
                self.master.after(0, self._stop_stream)
                return

            # Store hwnd for later resize
            ...
            if frame_width <= 0 or frame_height <= 0:
                frame_width, frame_height = 400, 600  # fallback

            # Re-parent scrcpy window into our Tk frame
            ctypes.windll.user32.SetParent(hwnd, frame_id)

            # Apply child window style (no caption/border)
            GWL_STYLE = -16
            WS_CHILD = 0x40000000
            WS_VISIBLE = 0x10000000
            WS_CAPTION = 0x00C00000
            WS_BORDER = 0x00080000
            ...
            # First resize to fill the frame
            ...
            # Bind resize event so scrcpy follows frame size
            def _bind_on_main_thread():
                if not self._stream_resize_bind_id:
                    self._stream_resize_bind_id = self.stream_embed_frame.bind(
                        "<Configure>", self._resize_stream_to_frame
                    )

            self.master.after(0, _bind_on_main_thread)
            ...
            if self.is_running:
                self.master.after(
                    0,
                    self.stream_status_label.config,
                    {"text": "Error: Failed to embed stream."}
                )

    def _resize_stream_to_frame(self, event=None):
        """Resize scrcpy window to always fill embed frame."""
        if not self.stream_hwnd:
            return
    ...
    def _stop_stream(self):
        ...
        # Clear stored window handle
        self.stream_hwnd = None

        # Unbind resize handler if set
    ...
    def on_app_quit(self):
        ...
        # Save current log content once more
        try:
            content = self.api_log_text.get("1.0", "end-1c")
            threading.Thread(
                target=self._save_log_to_file_worker,
    ...
    if os.path.exists(lock_file):
        try:
            ...
            if psutil.pid_exists(pid):
                messagebox.showinfo(
                    "Already Running",
                    "HHT Android Connect is already running in the system tray."
                )
                sys.exit()
            else:
